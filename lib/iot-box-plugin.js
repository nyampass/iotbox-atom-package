...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'iot-box-plugin:npm-update': () => this.npmUpdate(),
      'iot-box-plugin:rpi-setup': () => this.rpiSetup(),
    }));
  ...
  rpiSetup() {
    const ip = atom.config.get('iot-box-plugin.ip') || '192.168.1.10'
    const setupConfig = atom.config.get('iot-box-plugin.setup')
    new RpiSetup(setupConfig.hostname, setupConfig.ssid, setupConfig.password).run()
  }
};
