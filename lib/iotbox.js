...
export default {
  ...
  getClient() {
    ...
    const ip = atom.config.get('iotbox.ip') || '192.168.1.10'
  ...
  rpiSetup() {
    ...
    if (!setupConfig.ssid || !setupConfig.password) {
      atom.confirm({message: "IoTBoxパッケージの設定で、Wifi情報を入力してください"})
      return
    }

    this.view.logger.message("Start Raspbery Pi setup...")
    this.view.logger.message("connectiong..." + ip)

    new RpiSetup(this.view.logger, ip, setupConfig.ssid, setupConfig.password).run()
  }
};
